...
// bug report
function openBugReportModel() {
  const design = `
    <div class="d-flex flex-column gap-3">
      <textarea id="feedback" cols="30" rows="2" class="form-control" placeholder="Add your feedback..."></textarea>
      <button class="alg-btn-pill" data-bs-dismiss="modal" aria-label="Close" onclick="sendBugReport()">REPORT</button>
    </div>
  `;

  ALG.openModel("Bug Report", design, "&nbsp;");
}

function sendBugReport() {
  const bug = document.getElementById("feedback").value;

  fetch("api/bugReport.php?bug=" + bug, {
    method: "GET",
  })
    .then((res) => res.json())
    .then((data) => {
