...
async function openProductItemEditModel(
  ...
  productItemsData.forEach((element) => {
    if (element.product_item_id === id) {
      ...
      if (element.images.length != 0) {
        imagesDesgin += `<input style="width: 150px" type="file" onchange="editProductItemImage(event, '../../resources/images/singleProductImg/productId=${productId}&&weightId=${weightId}&&image=${count}.jpg', '${id}')" class="btn btn-secondary"/>`;
      }
    }
  });
